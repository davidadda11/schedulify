...
<script>
  import { onMount } from 'svelte';
  import { goto } from '$app/navigation'; // <-- Importat pentru redirecționare
  import { authClient } from '$lib/auth/auth-client';
  ...
  async function handleSubmit() {
    ...
    if (error) {
      ...
      showSuccess = true;
      // Opțional: Îl duce automat pe dashboard după 2 secunde
      setTimeout(() => {
        goto('/app/dashboard');
      }, 2000);
    }
    ...
    <div class="card-wrap">
      ...
      {#if showSuccess}
        <div class="success-state">
          ...
          <h2>Welcome aboard!</h2>
          <p>Your account has been created. Let's get scheduling.</p>
          <button class="btn-primary" onclick={() => goto('/app/dashboard')}>
            Go to Dashboard →
          </button>
        </div>
      {:else}
